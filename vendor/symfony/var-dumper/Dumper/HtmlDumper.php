...
class HtmlDumper extends CliDumper
{
    ...
    protected static $themes = [
        'dark' => [
            ...
            'note' => 'color:#1299DA',
            'ref' => 'color:#A0A0A0',
            'public' => 'color:#ebefd4FFF',
            'protected' => 'color:#ebefd4FFF',
            'private' => 'color:#ebefd4FFF',
            'meta' => 'color:#B729D9',
            'key' => 'color:#56DB3A',
            'index' => 'color:#1299DA',
...
return function (root, x) {
    ...
    a('mouseover', function (a, e, c) {
        if (c) {
            ...
            try {
                refStyle.innerHTML = 'pre.sf-dump .'+a[0]+'{background-color: #B729D9; color: #ebefd4 !important; border-radius: 2px}';
            } catch (e) {
            }
        }
    });
...
EOHTML
        ...
        $line .=
            'pre.sf-dump .sf-dump-ellipsis-note{' . $this->styles['note'] . '}';
    ...
    public function dumpString(Cursor $cursor, string $str, bool $bin, int $cut)
    {
        if (
            '' === $str &&
            isset($cursor->attr['img-data'], $cursor->attr['content-type'])
        ) {
            $this->dumpKey($cursor);
            $this->line .= $this->style(
                'default',
                $cursor->attr['img-size'] ?? '',
                []
            );
            $this->line .=
                $cursor->depth >= $this->displayOptions['maxDepth']
                    ? ' <samp class=sf-dump-compact>'
                    : ' <samp class=sf-dump-expanded>';
    ...
    protected function style(string $style, string $value, array $attr = [])
    {
        ...
        if (
            isset($attr['file']) &&
            ($href = $this->getSourceLink($attr['file'], $attr['line'] ?? 0))
        ) {
            $attr['href'] = $href;
        }
        if (isset($attr['href'])) {
    ...
    protected function dumpLine(int $depth, bool $endOfValue = false)
    {
        ...
        if (
            $this->headerIsDumped !==
            ($this->outputStream ?? $this->lineDumper)
        ) {
